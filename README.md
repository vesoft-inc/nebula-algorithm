...
## Get Nebula Algorithm
 1. Build Nebula Algorithm
    ...
    ```
    After the above buiding process, the target file  `nebula-algorithm-2.5.0.jar` will be placed under `nebula-algorithm/target`.
...
* Option2: Call nebula-algorithm interface
   ...
   Now there are 11 algorithms provided in `lib` from `nebula-algorithm`, which could be invoked in a programming fashion as below:
